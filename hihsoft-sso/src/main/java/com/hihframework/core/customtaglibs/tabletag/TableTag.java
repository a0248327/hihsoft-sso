...
public class TableTag extends BodyTagSupport {
	...
	public int doStartTag() throws JspException {
		buffer = new StringBuffer("<table class=\"" + (StringHelpers.notNull(cssName) ? cssName : "data-grid") + "\" style=\"" + (StringHelpers.notNull(style) ? style : "")
				+ "\" cellspacing=\"0\" cellpadding=\"0\">\n");
		if (items == null) {
			return SKIP_BODY;
		}
		if (StringHelpers.isNull(var))
			var = "bean";
		it = items.iterator();
	...
	public void setItems(Object items) {
		// try {
		// this.items = ExpressionEvaluatorManager.evaluate("items",
		// items.toString(), Object.class, this, pageContext);
		// } catch (JspException e) {
		// }
		this.items = (List<?>) items;
		if (this.items == null)
			this.items = new ArrayList<Map<String, ?>>();
	}
