...
public class PageHelper {
	...
	public String getPageBreakStr(Page pageObject) {
		...
		for (remberStart = startPage; remberStart < startPage + viewPage; remberStart++) {
			if (remberStart == page) {
				pageBarBuffer.append("<font color=\"red\" face=\"arial,sans-serif\">");
				pageBarBuffer.append("<b>");
				pageBarBuffer.append(remberStart + "</b></font>&nbsp;");
			} else {
				pageBarBuffer.append("<a href=\"" + url + "page=" + remberStart);
				if (isHaveParam) {
					pageBarBuffer.append("&" + param);
				}
				pageBarBuffer.append("\">");
				pageBarBuffer.append("<font face=\"arial,sans-serif\" color=\"#000000\">");
				pageBarBuffer.append("<b>");
		...
		try {
			cfg.setDirectoryForTemplateLoading(new File(realPath + pageObject.getPageTemplatePath()));
			...
			if (display1) {
				root.put("firstPageStart", "<a href=\"" + firstPageBuffer.toString() + "\">");
			...
			if (display2) {
				root.put("prevPageStart", "<a href=\"" + perPageBuffer.toString() + "\">");
			...
			if (display3) {
				root.put("nextPageStart", "<a href=\"" + nextPageBuffer.toString() + "\">");
			...
			if (display4) {
				root.put("lastPageStart", "<a href=\"" + lastPageBuffer.toString() + "\">");
