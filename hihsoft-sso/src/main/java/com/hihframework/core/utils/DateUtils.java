...
public class DateUtils {
	...
	public final static String[] MonthEn = { "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December" };
	...
	public static String getTimeSlice(Timestamp startTime, Timestamp endTime) {
		...
		String dayStart = caleStart.get(Calendar.YEAR) + "年" + (caleStart.get(Calendar.MONTH) + 1) + "月" + caleStart.get(Calendar.DAY_OF_MONTH) + "日";
		String dayEnd = caleEnd.get(Calendar.YEAR) + "年" + (caleEnd.get(Calendar.MONTH) + 1) + "月" + caleEnd.get(Calendar.DAY_OF_MONTH) + "日";

		if (dayStart.equals(dayEnd)) {
			// 同一天
			rtn = caleStart.get(Calendar.HOUR_OF_DAY) + "点" + caleStart.get(Calendar.MINUTE) + "分-" + caleEnd.get(Calendar.HOUR_OF_DAY) + "点" + caleEnd.get(Calendar.MINUTE) + "分";
		} else {
			// 不在同一天
			rtn = (caleStart.get(Calendar.MONTH) + 1) + "月" + caleStart.get(Calendar.DAY_OF_MONTH) + "日" + "-" + (caleEnd.get(Calendar.MONTH) + 1) + "月"
	...
	/**
	 * 获得指定日期所在月最小时间，时间部分为“00:00:00.000” 例如：param:2004-08-20
	 * return:2004-08-01 00.00.00.000
	 *
	 * @param date
	 *                指定的日期
	 * @return 指定日期所在月的最小时间
	...
	/**
	 * 获得指定日期所在月的最大时间，时间部分为“23.59.59.999”
	 * 例如：param:2004-08-20,return:2004-08-31 23.59.59.999
	 *
	 * @param date
	 * @return
	 */
	 ...
	 * 如：2002
	 * -05-23至2002-07-15两个日期段之间的日期分隔列表是2002-05-23、2002-06-01、2002-07-15
	 * 注意：开始日期必须小于结束日期。
	 */
	...
	public static java.sql.Timestamp toCalendarDate(String date, String time) {
		if ((date != null) && (date.length() >= 9) && (time != null) && !(time.equals(""))) {
			time = time.trim();
			...
			java.sql.Timestamp d = new java.sql.Timestamp(cal.getTime().getTime());
	...
	public static Collection<String> getAllYearForDay(String yy, String mm, Collection<String> incol, int start, int len, String op) {
	...
	// Logger.getLogger(DateUtil.class);
