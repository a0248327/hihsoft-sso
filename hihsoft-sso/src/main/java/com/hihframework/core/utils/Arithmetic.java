...
/**
 * <p>
 * Title:处理数值运算的辅助类：加法、减法、乘法、 除数、四舍五入处理、格式化数值
 * </p>
 * <p>
 * Description:传入double值，再用BigDecimal来计算 float和double只能用来做科学计算或者是工程计算
 * </p>
 * 在商业计算中我们要用 java.math.BigDecimal 如果需要精确计算，非要用String来构造BigDecimal
 * <p>
...
public class Arithmetic {
	...
	private static final String[] NumberCnS = { "0", "一", "二", "三", "四", "五", "六", "七", "八", "九", "十" };

	private static final String[] NumberCnF = { "零", "壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖" };

	private static final String[] unit = { "", "拾", "佰", "仟", "万", "亿", "角", "分", "元", "整" };
	...
	/**
	 * 方法一 格式化数字：格式化成900,000,000方便财务数据对照
	 *
	 * @param v
	 *                ：被格式化的数值
	 * @return格式化后的数值
	...
	public static final String separateMoney(String moneyValue) {
		if (moneyValue == null || moneyValue.trim().equals(""))
			...
			String tempMoney = moneyValue.indexOf(".") > 0 ? moneyValue.substring(0, moneyValue.indexOf(".")) : moneyValue;
			...
			retValue = moneyValue.indexOf(".") > 0 ? retValue + "." + moneyValue.substring(moneyValue.indexOf(".") + 1) : retValue;
	...
	public static final String moneyCn(String money) {
		...
		for (int i = 0; i < length; i++) {
			...
			int c = (length - 1 - i) % 4;
			if (temp[i] != '0') {
				b = b + NumberCnF[Integer.parseInt(String.valueOf(temp[i]))] + unit[c];
		...
		if (!fraction.equals("")) {
			...
			if (e[0] == '0' && e[1] != '0')
				...
				b = b + NumberCnF[Integer.parseInt(String.valueOf(e[0]))] + unit[6];
			if (e[1] != '0')
				b = b + NumberCnF[Integer.parseInt(String.valueOf(e[1]))] + unit[7];
		}
		return b + unit[9];
	}
