...
/**
 * <p>
 * Title：利用freemarker创建HTML静态页面的辅助类
 * </p>
 * <p>
 * Description:进一步封装此类，成为javahih核心类成员
 * </p>
 * <p>
...
public class MakeTemplateFileByFM {
	private static final Logger log = Logger.getLogger(MakeTemplateFileByFM.class.getName());
	...
	public MakeTemplateFileByFM(Object obj, FreeMarkerDTO dto, String templatePath, String savePath, HttpServletRequest request) {
		String realPath = dto.getRealPath();
		String templatepath = realPath + templatePath;
		/************************** 构造文件的存放位置及生成各种类型的文件命名规则 ****************/
		String cDateStr = savePath + File.separator + DateUtils.getDateFormatStr("yyyyMMdd");
		...
		try {
			...
			root.put("JspTaglibs", new TaglibFactory(request.getSession().getServletContext())); // 加载struts标签库
			Writer out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileName), "UTF-8"));// 设定编码，避免乱码
			try {
	...
	public static MakeTemplateFileByFM getInstance(Object obj, FreeMarkerDTO vo, String templatePath, String savePath, HttpServletRequest request) {
		synchronized (lock) {
			if (null == instance) {
				log.info("没有此实例，正在实例化一个freeMarker模板类");
				instance = new MakeTemplateFileByFM(obj, vo, templatePath, savePath, request);
			}
