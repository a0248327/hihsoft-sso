...
public class BaseExport {
	...
	protected BigDecimal strToBigDec(String strData) {
		...
		try {
			tmp = ((strData == null || strData.trim().equals("")) ? new BigDecimal(0) : new BigDecimal(strData.trim()));
	 ...
	 *                要插入的字符串
	 */
	protected boolean addString(int column, int row, String label) throws ExcelExportException {
	 ...
	 *                要插入的字符串
	 */
	public boolean addStringNOBorder(int column, int row, String label) throws ExcelExportException {
	...
	public boolean addNumber(int column, int row, double number) {
		...
		if (wsheet != null) {
			try {
				...
				jxl.write.Number labelN = new jxl.write.Number(column, row, number, wcf_right);
		...
		if (wsheet != null) {
			try {
				if (number != null) {
					...
					jxl.write.Number labelN = new jxl.write.Number(column, row, number.doubleValue(), wcf_right);
					wsheet.addCell(labelN);
				}
			} catch (RowsExceededException ree) {
				ree.printStackTrace();
	...
	public boolean addBoolean(int column, int row, boolean bule) {
		...
		if (wsheet != null) {
			...
			try {
				jxl.write.Boolean labelB = new jxl.write.Boolean(column, row, bule);
	...
	public boolean addFormateDateTime(int column, int row, Date date) {
		if (wsheet != null) {
			try {
				...
				jxl.write.DateTime labelDT = new jxl.write.DateTime(column, row, date, wcfDF);
				wsheet.addCell(labelDT);
			} catch (RowsExceededException ree) {
				ree.printStackTrace();
				return false;
	...
	public boolean addFormatLabelNumber(int column, int row, double number) {
		if (wsheet != null) {
			try {
				...
				jxl.write.Number labelNT = new jxl.write.Number(column, row, number, wcfN);
	 ...
	 *                结束行
	 */
	public boolean mergeCells(int beginColumn, int beginRow, int endColumn, int endRow) {
		if (wsheet != null) {
			try {
