...
public class ExcelUtil {
	 ...
	 * @return 创建过的工作表
	 */
	public WritableSheet createWritableSheet(WritableWorkbook wwb, String sheetName, int sheetNo) throws Exception {
	 ...
	 * @throws Exception
	 */
	public WritableSheet getWsheet(WritableWorkbook wwb, String sheetName) throws Exception {
	 ...
	 * @throws Exception
	 */
	public WritableSheet getWsheet(WritableWorkbook wwb, int sheetNo) throws Exception {
	 ...
	 *                要插入的字符串
	 */
	public void addString(WritableSheet wsheet, int column, int row, String label) throws Exception {
	 ...
	 *                要插入的数值
	 */
	public void addNumber(WritableSheet wsheet, int column, int row, double number) {
	 ...
	 *                要插入的布尔型的值
	 */
	public void addBoolean(WritableSheet wsheet, int column, int row, boolean bolean) {
		try {
			jxl.write.Boolean labelB = new jxl.write.Boolean(column, row, bolean);
	...
	public void addDateTime(WritableSheet wsheet, int column, int row, Date date) {
		try {
			jxl.write.DateTime labelDT = new jxl.write.DateTime(column, row, date);
	...
	/**
	 * 将数据库中的数据导入到工作表 File file=new File("c:/aaa.xls"); ExcelUtil eu=new
	 * ExcelUtil(); eu.batchWriteSheet(file,"bbb",0,0,list);
	 *
	 * @param file
	 *                要生成的excel文件
	 * @param sheetName
	...
	public static void main(String[] args) throws Exception {
		...
		// --------------------------------------读文件-------------------------------------
		/*
		 * File file = new File("d:/收入明细表.xls"); ExcelUtil eu = new
		 * ExcelUtil(); //创建可读的工作表对象，定位到要读取的excel文件 Workbook workbook =
		 * eu.createWorkbook(file); //读取此文件的第一个工作表，工作表序号从0开始。 Sheet
		 * sheet = eu.getSheet(workbook,0); //也可以通过工作表名称来读取此工作表 //Sheet
		 * sheet = eu.getSheet(workbook,"收入明细表");
		 ...
		 * eu.getRowContentList(sheet, 0); for (int ii = 0; ii <
		 * rowlist.size(); ii++) { System.out.println(rowlist.get(ii));
		 * } System.out.println(eu.getSheetName(sheet));//得到此工作表名称
		 * //得到第一列第八行单元格内容 System.out.println(eu.getContents(sheet, 0,
		 * 8));
		 */
