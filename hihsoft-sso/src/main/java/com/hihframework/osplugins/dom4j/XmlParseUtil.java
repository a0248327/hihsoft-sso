...
public class XmlParseUtil {
	...
	public void writeXml(Document document) {
		try {
			...
			// XMLWriter output = new XMLWriter(new
			// FileWriter(file));
			output.write(document);
			output.flush();
			output.close();
	 ...
	 * @param NodeName要更改的元素名的上级节点名
	 *                ，格式为“//父元素名”， 例如：//article，表示article元素
	 * @param element
	 *                要更改的元素名
	 */
	 ...
	 * @param nodesName
	 *                节点名称，
	 *                格式："//节点名称"，如果是多个节点下的，就一直加“/节点名称",此节点名称是遍历拥有多个子节点的。
	 * @return 此节点下所有节点信息。
	 */
	public List<?> getNodes(Document document, String nodesName) {
		 ...
		 * Attribute attribute = (Attribute) iter.next();
		 * //然后操作attribute String url = attribute.getValue(); }
		 */
		/*
		 * for (Iterator iter = list.iterator(); iter.hasNext(); ) {
		 * Element element = (Element) iter.next(); //然后操作element }
		 */
		return list;
	}
	...
	public static void main(String[] args) throws DocumentException, MalformedURLException {
		 ...
		 * xp.createRootElement(doc, "学生信息");//创建根节点
		 * xp.addComment(rootElement, "学生信息管理");//创建根节点注释 //创建三好学生信息节点
		 * Element goodstudElement = xp.addElement(rootElement,
		 * "三好学生信息"); //创建一般学生信息节点 Element plainElement =
		 * xp.addElement(rootElement, "一般学生信息"); //为三好学生信息节点创建学生子节点
		 * Element studentElement = xp.addElement(goodstudElement,
		 * "学生"); //为学生节点创建性别属性和年龄属性，分别对应男和23的值。
		 * xp.addAttribute(studentElement, "性别", "男");
		 * xp.addAttribute(studentElement, "年龄", "23"); //为学生节点创建姓名子节点
		 * Element nameElement = xp.addElement(studentElement, "姓名");
		 * //为姓名节点创建钟裔文本值 xp.setText(nameElement, "钟裔");
		 * //为学生节点创建期末考试总分子节点 Element scoreElement =
		 * xp.addElement(studentElement, "期末考试总分"); //为期末考试总分节点创建600文本值
		 * xp.setText(scoreElement, "600"); //为学生节点创建班主任子节点 Element
		 * techerElement = xp.addElement(studentElement, "班主任");
		 * //为班主任节点创建王老五文本值 xp.setText(techerElement, "王老五");
		 * //为学生节点创建平时表现子节点 Element actElement =
		 * xp.addElement(studentElement, "平时表现");
		 * //为平时表现节点创建年度和班级属性，对应2004和计科2班的值。 xp.addAttribute(actElement,
		 * "生活表现", "优秀"); xp.addAttribute(actElement, "学习表现", "优秀");
		 * xp.addAttribute(actElement, "运动表现", "一般"); Element
		 * totalElement = xp.addElement(actElement, "综合表现");
		 * //为综合表现节点创建良好文本值 xp.setText(totalElement, "良好"); Element
		 * pjiaElement = xp.addElement(actElement, "综合评价");
		 * //为综合评价节点创建良好文本值 xp.setText(pjiaElement, "优秀");
		 * //创建文件名为catalog.xml File file = new File("c:/catalog.xml");
		 * //写入文件，并格式化，解决中文问题。 xp.writeXml(doc, file);
		 */

		// ----------------------解析读取修改XML文件--------------------------
		/*
		 * File file = new File("c:/catalog.xml"); XmlParseUtil xp = new
		 * XmlParseUtil(); //创建文档读写对象 Document document =
		 * xp.readXml(file); //取得ROOT根节点 Element rootElement =
		 * xp.getRootElement(document); //取得根节点名称 String rootName =
		 * xp.getRootName(rootElement); log.info(rootName); //
		 * 用列表的方式获取根节点下所有子节点的名称列表 ArrayList elementNameList =
		 * xp.getElementNameList(rootElement); if
		 * (!elementNameList.isEmpty()) { for (int i = 0; i <
		 * elementNameList.size(); i++) { //输出根节点下一级子节点的名称
		 * log.info((String) elementNameList.get(i)); } }
		 * //用迭代的方式获取根节点下子节点的名称 Iterator iterator =
		 * xp.elementIterator(rootElement);
		 * 
		 * while (iterator.hasNext()) { //迭代下一级子节点 Element stuelement =
		 * (Element) iterator.next(); log.info(stuelement.getName());
		 * //迭代stuelement的子节点 Iterator stuiterator =
		 * xp.elementIterator(stuelement); while (stuiterator.hasNext())
		 * { Element studentelement = (Element) stuiterator.next();
		 * //根据下级节点的属性名取得属性值 String saleAttrValue =
		 * xp.getAttributeValue(studentelement, "性别");
		 * log.info(saleAttrValue); String ageAttrValue = xp
		 * .getAttributeValue(studentelement, "年龄");
		 * log.info(ageAttrValue); //通过哈希表方式取出属性和属性值 HashMap
		 * attributeMap = (HashMap) xp .attributeMap(studentelement);
		 * //......取出HashMap中属性和属性值的操作
		 * 
		 * //取出学生节点下所有子节点的名称 ArrayList studentelementList = xp
		 * .getElementNameList(studentelement); if
		 * (!studentelementList.isEmpty()) { for (int i = 0; i <
		 * studentelementList.size(); i++) {
		 * log.info(studentelementList.get(i)); } } //输出学生节点的下级子节点数量
		 * log.info(xp.getElementCount(studentelement)); //输出学生节点的属性数量
		 * log.info(xp.getAttributeCount(studentelement));
		 * //修改学生节点性别属性的值为女 xp.setAttributeValue(studentelement, "性别",
		 * "女"); //输出学生节点的性别属性值
		 * log.info(xp.getAttributeValue(studentelement, "性别"));
		 * xp.setElementName(studentelement, "学生"); //遍历学生节点所有子节点
		 * Iterator elementiterator =
		 * xp.elementIterator(studentelement); while
		 * (elementiterator.hasNext()) { //取出每个子节点 Element element =
		 * (Element) elementiterator.next(); //判断子节点的节点数量 int count =
		 * xp.getElementCount(element); //如果子节点是根节点，则输出节点文本值 if (count
		 * == 0) { String elementText = xp.getElementText(element);
		 * log.info(elementText); } else { //遍历学生节点属性值 Iterator
		 * attrIterator = xp.attributeIterator(element); if
		 * (attrIterator.hasNext()) { //遍历属性，输出属性和属性值 Attribute
		 * attribute = (Attribute) attrIterator .next();
		 * log.info(attribute.getName());
		 * log.info(attribute.getValue());
		 * log.info(xp.getAttributeCount(element)); } Iterator
		 * elemIterator = xp.elementIterator(element); if
		 * (elemIterator.hasNext()) { Element bxelement = (Element)
		 * elemIterator.next(); String elementText =
		 * xp.getElementText(bxelement); log.info(elementText); } } }
		 * 
		 * } }
		 * 
