...
/**
 * Title:角色控制层 Description: Copyright: Copyright (c) 2013
...
public class TaclRoleController extends javahihBaseController {
	...
	public ModelAndView list(HttpServletRequest request, HttpServletResponse response) throws ControllerException {
		...
		/*
		 * if(StringHelpers.notNull(rolesort)){ hql +=
		 * " and t.rolesort = '"+rolesort+"'";
		...
		if (StringHelpers.notNull(user.getOrgid()) && falg) {
			if (isGroup) {
				...
				hql += " and (t.orgid in('" + orgids + "') and t.roleSort=2) or (t.roletype in(" + getRoleTypeLower(request, response) + ") and t.roleSort=1)";
			}
		}
		...
		List<TaclRole> list = (List<TaclRole>) taclRoleService.getPageDataByHQL(hql, Integer.parseInt(pageSize), Integer.parseInt(pageNo));
	...
	public ModelAndView listRole(HttpServletRequest request, HttpServletResponse response) throws ControllerException {
	...
	public ModelAndView modifyInit(HttpServletRequest request, HttpServletResponse response) throws ControllerException {
		boolean isGroup = isGroup(request, response);
	...
	public ModelAndView userRole(HttpServletRequest request, HttpServletResponse response) throws ControllerException {
		...
		if (StringHelpers.notNull(user.getOrgid())) {
			hql += " and t.orgid in (" + baseService.getPrivilege(user.getUserid()) + ")";
		}
		...
		if (pageNo == null)
			pageNo = "1";
		/*
		 * List<TaclUserinfo> list =
		 * taclUserinfoService.getPageDataByHQL(hql, new Object[] {},
		...
		// int rows = Integer.parseInt(pageSize);
		// int pages = total % rows == 0 ? (total / rows) : (total /
		// rows + 1);
	...
	public ModelAndView authorization(HttpServletRequest request, HttpServletResponse response) throws ControllerException {
		String ids = request.getParameter("ids"); // 用户ID
		String roleId = request.getParameter("roleId"); // 角色ID
		List<TaclRoleuser> roleList = taclRoleService.getTaclRoleUserByRoleId(roleId);
		if (ids != null && !"".equals(ids)) {
			String arr[] = ids.split(","); // 拆分多个用户ID
			for (String userId : arr) {
				boolean found = false;
				for (TaclRoleuser taclUser : roleList) {
					if (taclUser.getUserid().equals(userId) && taclUser.getRoleid().equals(roleId)) {
						found = true;
	...
	public ModelAndView cancel(HttpServletRequest request, HttpServletResponse response) throws ControllerException {
		String ids = request.getParameter("ids"); // 用户ID
		String roleId = request.getParameter("roleId"); // 角色ID
		if (ids != null && !"".equals(ids)) {
			String arr[] = ids.split(",");
			for (String userId : arr) {
				String hql = "from TaclRoleuser where userid = '" + userId + "' and roleid ='" + roleId + "'";
				@SuppressWarnings("unchecked")
				List<TaclRoleuser> roleUser = (List<TaclRoleuser>) taclRoleuserService.getTaclRoleuserByHQL(hql);
	...
	public ModelAndView userView(HttpServletRequest request, HttpServletResponse response) {
		...
		for (TaclRoleuser taclUserinfo : roleList) {
			TaclUserinfo userInfo = taclUserinfoService.getTaclUserinfoById(taclUserinfo.getUserid());
	...
	public ModelAndView view(HttpServletRequest request, HttpServletResponse response) throws ControllerException {
	...
	public ModelAndView save(HttpServletRequest request, HttpServletResponse response) throws ControllerException {
		TaclRole taclRole = new TaclRole();
		TaclUserinfo user = (TaclUserinfo) request.getSession().getAttribute(Consts.USER_INFO);
		...
		boolean flag = taclRoleprivilegeService.saveRoleSet(taclRole.getRoleid(), moduleSet);
	...
	public ModelAndView delete(HttpServletRequest request, HttpServletResponse response) throws ControllerException {
		...
		if (ids != null && !"".equals(ids)) {
			...
			for (String id : arr) {
				...
				if (roleList.size() == 0) {
					...
					obj.setRoleid(id);
					String hql = "delete from TaclRoleprivilege where roleid='" + id + "'"; // 删除角色时候删除树结构
