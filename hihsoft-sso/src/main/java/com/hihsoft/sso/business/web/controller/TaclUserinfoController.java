...
/**
 * Title: Description:用戶基本信息的服務 Copyright: Copyright (c) 2013
...
public class TaclUserinfoController extends javahihBaseController {
	...
	public ModelAndView listRole(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (isGroup(request, response)) {
			...
			hql += " and (t.orgid in('" + getOrgId(request) + "') and t.roleSort=2) or (t.roletype in(" + getRoleTypeLower(request, response) + ") and t.roleSort=1)";
		}
	...
	public ModelAndView saveRoleSet(HttpServletRequest request, HttpServletResponse response) throws Exception {
		String userId = getParam(request, "userId");
	...
	public ModelAndView getOrgTree(HttpServletRequest request, HttpServletResponse response) throws ControllerException {
		...
		try {
			...
			// List<TsysDataprivilege> assigned =
			// baseService.getValueObjectsByHQL(hql, userId);
