...
/**
 * Title:系统参数的控制层的服务 Description: Copyright: Copyright (c) 2013
...
public class TsysParameterController extends javahihBaseController {
		...
		for (Iterator<String> it = set.iterator(); it.hasNext();) {
			...
			if (StringHelpers.notNull(value)) {
				if (key.equals("paraType")) {
					hql += " and UPPER(" + key + ") like '%" + StringHelper.toUpperCase(value.toString()) + "%'";
					hql += " and LOWER(" + key + ") like '%" + StringHelper.toLowerCase(value.toString()) + "%'";
				} else
	...
	public ModelAndView modify(HttpServletRequest request, HttpServletResponse response) throws ControllerException {
		...
		TsysParameter tsysParameter = tsysParameterService.getTsysParameterById(id);
