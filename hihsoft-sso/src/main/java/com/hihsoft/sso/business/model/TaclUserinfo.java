...
public class TaclUserinfo extends BaseEntity implements IUser {
	...
	public TaclUserinfo(java.lang.String userid) {
		this.userid = userid;
	}
	...
	public String toString() {
		return new ToStringBuilder(this).append("Userid", getUserid()).append("Birthday", getBirthday()).append("Certified", getCertified()).append("City", getCity())
				.append("UserEmail", getUserEmail()).append("Idcard", getIdcard()).append("Imagepath", getImagepath()).append("Isonline", getIsonline())
				.append("Loginname", getLoginname()).append("Mobile", getMobile()).append("Msn", getMsn()).append("Nickname", getNickname()).append("Province", getProvince())
				.append("Qq", getQq()).append("Sex", getSex()).append("Orgid", getOrgid()).append("Deptid", getDeptid()).append("Telephone", getTelephone())
				.append("Userdesc", getUserdesc()).append("Username", getUsername()).append("Userpw", getUserpw()).append("Userstate", getUserstate())
				.append("Usertype", getUsertype()).append("Work", getWork()).append("Trade", getTrade()).append("Age", getAge()).append("UserNo", getUserNo()).toString();
	}

	public int hashCode() {
		return new HashCodeBuilder().append(getUserid()).append(getBirthday()).append(getCertified()).append(getCity()).append(getUserEmail()).append(getIdcard()).append(getImagepath())
				.append(getIsonline()).append(getLoginname()).append(getMobile()).append(getMsn()).append(getNickname()).append(getProvince()).append(getQq()).append(getSex())
				.append(getOrgid()).append(getDeptid()).append(getTelephone()).append(getUserdesc()).append(getUsername()).append(getUserpw()).append(getUserstate())
				.append(getUsertype()).append(getWork()).append(getTrade()).append(getAge()).append(getUserNo()).toHashCode();
	}

	public boolean equals(Object obj) {
		if (obj instanceof TaclUserinfo == false)
			return false;
		if (this == obj)
			return true;
		TaclUserinfo other = (TaclUserinfo) obj;
		return new EqualsBuilder().append(getUserid(), other.getUserid()).append(getBirthday(), other.getBirthday()).append(getCertified(), other.getCertified())
				.append(getCity(), other.getCity()).append(getUserEmail(), other.getUserEmail()).append(getIdcard(), other.getIdcard()).append(getImagepath(), other.getImagepath())
				.append(getIsonline(), other.getIsonline()).append(getLoginname(), other.getLoginname()).append(getMobile(), other.getMobile()).append(getMsn(), other.getMsn())
				.append(getNickname(), other.getNickname()).append(getProvince(), other.getProvince()).append(getQq(), other.getQq()).append(getSex(), other.getSex())
				.append(getOrgid(), other.getOrgid()).append(getDeptid(), other.getDeptid()).append(getTelephone(), other.getTelephone()).append(getUserdesc(), other.getUserdesc())
				.append(getUsername(), other.getUsername()).append(getUserpw(), other.getUserpw()).append(getUserstate(), other.getUserstate())
				.append(getUsertype(), other.getUsertype()).append(getWork(), other.getWork()).append(getTrade(), other.getTrade()).append(getAge(), other.getAge())
				.append(getUserNo(), other.getUserNo()).isEquals();
	}

	public boolean isAdmin() {
		List<TsysParameter> list = ParamUtil.getInstance().getByType("SUPER_USER");
		for (TsysParameter p : list) {
			if (p.getParano().equals(loginname))
				return true;
		}
		return false;
	}
