...
/**
 * Title:模块定义的控制层服务 Description: Copyright: Copyright (c) 2013
...
public class TsysModuleinfoController extends javahihBaseController {
	...
	public ModelAndView queryModule(final HttpServletRequest request, final HttpServletResponse response) throws ControllerException {
		Map<String, Object> filter = WebUtils.getParametersStartingWith(request, "srh_");// 得到查询条件
	...
	public ModelAndView getTree(HttpServletRequest request, HttpServletResponse response) throws ControllerException {
	...
	public ModelAndView saveOrUpdate(HttpServletRequest request, HttpServletResponse response) throws Exception {
	...
	public ModelAndView modify(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		TsysModuleinfo moduleinfo = tsysModuleinfoService.getTsysModuleinfoById(id);
	...
	public ModelAndView queryList(final HttpServletRequest request, final HttpServletResponse response) throws Exception {
		...
		if (pageNo == null)
			pageSize = "1";
		final List list = tsysModuleinfoService.getTsysModuleinfoPageDataByHQL(hql, new Object[] {}, Integer.parseInt(pageSize), Integer.parseInt(pageNo));
		final int total = tsysModuleinfoService.getDataTotalNum(hql);
		final String result = "{\"total\": " + total + ", \"rows\": " + JsonUtil.toString(list, JsonUtil.COLLECTION_FILTER) + "}";
