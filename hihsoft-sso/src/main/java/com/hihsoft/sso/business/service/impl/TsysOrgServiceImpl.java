...
public class TsysOrgServiceImpl extends BaseServiceImpl implements TsysOrgService {
		...
		baseDAO.doInHibernate(new HibernateCallback<Void>() {
			public Void doInHibernate(Session session) throws HibernateException, SQLException {
				...
				if (StringHelpers.notNull(tsysOrg.getOrgid())) {
					...
					List<TsysOrg> tsysOrgs = (List<TsysOrg>) session.createQuery(
							" from TsysOrg where orgno = '" + tsysOrg.getOrgno() + "' and orgid != '" + tsysOrg.getOrgid() + "'").list();
					if (tsysOrgs.size() > 0) {
	 ...
	 * @throws DataAccessException
	 */
	public List<?> getTsysOrgPageDataByMap(Map<String, Object> filter, int page_size, int pageNo) throws ServiceException {
		return baseDAO.getPageDataByHQL(QUERY_TSYSORG_HQL.toString(), filter, page_size, pageNo);
	}
	 ...
	 */
	public List<?> getTsysOrgValueObjectByDetachedCriteria(DetachedCriteria detachedCriteria) throws ServiceException {
	 ...
	 * @throws ServiceException
	 */
	public List<?> getTsysOrgValueObjectByDetachedCriterias(DetachedCriteria detachedCriteria, int arg1, int arg2) throws ServiceException {
