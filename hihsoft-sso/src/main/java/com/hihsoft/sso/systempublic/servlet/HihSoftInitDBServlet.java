 ...
 * 原则上先启动Spring上下文、再调用此初始化类，然后执行spring mvc上下文 Copyright:Copyright (c) 2013
...
public class HihSoftInitDBServlet extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
	...
	public void init() throws ServletException {
		...
		try {
			...
			String[] folderPaths = HihSoftInitDBServlet.getParamValue(Constant.SYS_CONFIGlOCATION_XML_NAME).split(",");
			String path = null;
			// 循环读取文件
	...
	private void readFile(File file) throws Exception {
		...
		} else if (name != null && name.indexOf("hihsoft") == 0 && name.substring(name.lastIndexOf(".") + 1, name.length()).toUpperCase().equals("XML")) {
			loadFile(file);
		} else {
		...
		while (it.hasNext()) {
			...
			if (el.getName().equals("import") && isLoad != null && isLoad.equals("true")) {
				...
				if (file1.isDirectory()) {
					...
					// 如果是初始化框架
					if (el.getName().equals("import") && isLoad != null && isLoad.equals("true") && type != null && type.equals("framework")) {
						initDB(doc1);
	...
	public boolean accept(File dir, String name) {
		boolean a = false;
		if (name != null && name.indexOf("hihsoft") == 0 && name.substring(name.lastIndexOf(".") + 1, name.length()).toUpperCase().equals("XML")) {
			a = true;
			// 检查下程序目录下是否有此.xml文件，
		}
		return a;
