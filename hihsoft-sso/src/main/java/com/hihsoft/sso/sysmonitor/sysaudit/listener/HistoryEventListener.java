 ...
 * 经调试发现：记录审计日志的必须调用spring封装的saveOrMergeVO方法才行. 同时要注意配置文件中涉及到外键的情况lazy=false才行
 * 同时业务对象必须继承BaseAuditEntity
 * 奇怪在PostUpdateEventListener中save操作却不能成功？很难获取到hibernate的session
 * ，不能进行持久化操作.单独从会话工厂中获取session
 * 
 * @author hihsoft
 */
public class HistoryEventListener extends DefaultLoadEventListener implements PostUpdateEventListener {
	...
	public void onPostUpdate(PostUpdateEvent event) {
		log.info("开始监控修改的对象" + event.getEntity().getClass().getName() + "----------------->>>>>>>" + event.getEntity().toString());
			...
			for (int i = 0; i < event.getState().length; i++) {
				...
				if (oldValue != null && !oldValue.equals(newValue)) {
					...
					if (oldValue instanceof AuditableEntity) {
						entry.setFrontData(((AuditableEntity) oldValue).getId().toString());
					} else {
						entry.setFrontData(oldValue != null ? oldValue.toString() : null);
					}
					if (newValue instanceof AuditableEntity) {
						entry.setBehindData(((AuditableEntity) newValue).getId().toString());
					} else {
						entry.setBehindData(newValue != null ? newValue.toString() : null);
					}
