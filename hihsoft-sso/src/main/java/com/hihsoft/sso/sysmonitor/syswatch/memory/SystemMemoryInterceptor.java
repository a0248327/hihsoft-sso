...
public class SystemMemoryInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		if (enabled) {
			...
			float b = (float) (a);
			logger.info("JVM内存分配: 额定" + (float) ((float) Runtime.getRuntime().maxMemory() / b) + "MB 实际" + (float) (Runtime.getRuntime().totalMemory() / b) + "MB 剩余"
					+ (float) Runtime.getRuntime().freeMemory() / b + "MB");
		}
